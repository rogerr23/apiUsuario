...
public class JwtTokenComponent {
	...
	// Metodo para gerar um TOKEN JWT para um usuario autenticado
	public String generateToken(Usuario usuario) throws Exception {
		...
		return Jwts.builder().setSubject(usuario.getEmail()).setNotBefore(dataAtual)
				.setExpiration(new Date(dataAtual.getTime() + expiration))// tempo de expiração do token
				.signWith(SignatureAlgorithm.HS256, "apiusuarios-cotiinformatica").compact();
